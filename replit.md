...
- **UI Improvements** (November 2025): Removido horário de "Últimas Movimentações" em VehicleDetails (agora mostra apenas data dd/mm/yyyy). Sistema de "Observações Gerais da Loja" nos Relatórios refatorado para funcionar exatamente igual ao "Status do Checklist": botões abrem modal Dialog ao invés de mostrar lista inline, com mesmo padrão visual e comportamento (states observationsDialogOpen/observationsDialogType, filtros por Pendentes/Resolvidas).
